...
function onNavigatingTo(args) {
    ...
    *************************************************************/
    ...
    var album_art_img = page.getViewById("album-art-img");
    
    var albumart_source = "~/images/" + contextData.AlbumId + ".jpg";

    pageData.set("albumname", contextData.AlbumName);
    pageData.set("artistname", contextData.Artist);
    pageData.set("albumart", albumart_source);
...
function postPurchaseInvoiceLine(item, invoiceId)
{
    ...
    fetch(odata_URL, init).then(function (response) {
        ...
        return response.json().then(function(json){
            ...
            {
                max_invoiceLineId = invoiceLine[0].InvoiceLineId;
            }
            ...
            logger.logRequest(odata_URL, init.method, init.body);
            ...
            fetch(odata_URL, init).then(function (response) {
                ...
                return response.json().then(function(json){
                    ...
                    var returnData = json;
                    dialogs.alert("Purchase Successful!").then(function() {
...
function getCustomerData(item)
{
    ...
    if(!odata_URL.endsWith("/"))
    {
        odata_URL = odata_URL + "/Customers(CustomerId="+cust_id+")";
    } else
    {
        odata_URL = odata_URL + "Customers(CustomerId="+cust_id+")";
    }
